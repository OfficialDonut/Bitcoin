...
public enum Message {
    ...
    private static void loadDefaults() {
        ...
        Bitcoin.plugin.getUtil().saveYml(messagesFile, messagesConfig);
    }

    private static void loadMessages() {
        Arrays.stream(Message.values()).forEach(message -> {
            List<String> messageValue = messagesConfig.getStringList(message.path);
            messages.put(message, Bitcoin.plugin.getUtil().colorMessage(messageValue.isEmpty() ? messagesConfig.getString(message.path) : String.join("\n", messagesConfig.getStringList(message.path))));
        });
    }
